...
```{r}
lm1 <- linearModel(formula = form, data = data, print.summary = FALSE)
```

Here is an example of using `print.summary = TRUE`. This summary is of similar format to the output from the `lm` function.

```{r}
lm1 <- linearModel(formula = form, data = data, print.summary = TRUE)
```


The `linearModel` function returns a list containing coefficients, residuals, and fitted values from the specified linear model.
...
```{r}
#bench::mark(lm(form, data = iris), linearModel(formula = form, data = iris, print.summary = FALSE))
```
